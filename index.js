...
function interpolateVariables(template, context) {
    ...
    if (template.search(/{\s*\w+\s*}/) < 0) { return template }
    var regex = /{\s*(\S+?)\s*(?:"([^"\\]*(?:\\.[^"\\]*)*)"\s*)?}/
